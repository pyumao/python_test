...
class DoExcel:
    ...
    def read_data(self):#section 配置文件里面的片段名 可以根据你的指定来执行具体的用例
        '''从Excel读取数据，有返回值'''
        #从配置文件里面控制读取哪些用例
        case_id=ReadConfig().get('CaseAll','caseid')
        ...
        for key in eval(case_id):#遍历字典
            ...
            if eval(case_id)[key]=='1':#如果case_id==all 那就获取所有的用例数据
                final_data.extend(test_data)#把测试用例赋值给final_data这个变量
            else:#否则 如果是列表 那就获取列表里面指定id的用例的数据
                for i in eval(case_id)[key]:#遍历case_id 里面的值
...
if __name__ == '__main__':
    sheet_name='recharge'
    test_data=DoExcel(case_path,sheet_name).read_data()
    print(test_data)